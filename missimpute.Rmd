...
all three imputation will be performed row-wise (proteins as rows) and column-wise (proteins as cols) 
...
```{r}
rmse_df <- data.frame(rmse_rf_row, rmse_rf_col, rmse_knn_row, rmse_knn_col, rmse_left_row, rmse_left_col) %>% 
  mutate(iteration = seq_along(1:20)) %>% 
...
ggplot(data = rmse_df, aes(x=imputation, y = RMSE)) +
  geom_point(size = 3) +
  theme_bw()+
